...
void ira_retry(void)
{
    ...
    Ir_Receive_Data = 0;
    Ir_Receive_Count = 0;
    Ir_receive_ok = 0; //红外接收失败
}
void ira_finish(void)
{
...
{
    if (Ir_receive_ok) {
        printstr("there is noe un cpoe data\n");
        return;
    }
    if (start_flag)    {
        time1 = TIM2_GetCounter();  //第一个下降沿开始的时候，记录时刻1
        start_flag = 0;
        time_interval = 0;
        Ir_Status = 1;
        return;
    ...
    switch (Ir_Status) {
        case 1://第二个下降沿，定时器关闭，读取定时器计数值
            if ((time_interval >= 12500) && (time_interval <= 14500)) { //判断引导码是否正确9+4.5ms
                ...
                //printstr("Ir_Status 1 ira_retry get head:interval:");
                //printdec(time_interval);
                //printstr("\n");
				Ir_Status=1;
				Ir_Receive_Data=0;
				Ir_Receive_Count=0;
        ...
        case 2://开始32位数据的接收
            if ((time_interval >= 1000) && (time_interval <= 1300)) { //间隔1.12ms ->0
                ...
                //printstr("Ir_Status 2 ira_retry:interval:");
                //printdec(time_interval);
                //printstr("\n");
                ira_retry();
                goto end;
